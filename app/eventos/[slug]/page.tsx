...
function calcular_duracion_evento(
  fecha_inicio: string,
  fecha_fin: string,
): string {
...
export default async function DetalleEventoPage({
  ...
  const duracion_evento = calcular_duracion_evento(
    evento.fecha_inicio,
    evento.fecha_fin,
  );
  const es_evento_futuro = new Date(evento.fecha_inicio) > new Date();
  const es_evento_en_curso =
    new Date() >= new Date(evento.fecha_inicio) &&
    new Date() <= new Date(evento.fecha_fin);
            ...
            <div className="bg-white rounded-xl border p-8 mb-8">
              ...
              <div className="prose prose-lg prose-blue max-w-none">
                <p className="text-gray-700 leading-relaxed whitespace-pre-line">
                  {evento.descripcion ||
                    "Descripción no disponible para este evento."}
                </p>
              </div>
            </div>
